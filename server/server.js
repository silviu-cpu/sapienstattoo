...
app.get("/fetch-google-reviews", async (req, res) => {
  try {
    ...
    const pageSize = 10; // Specify the number of reviews to fetch per page

    let nextPageToken = null;
    let allReviews = [];

    do {
      const url = `https://maps.googleapis.com/maps/api/place/details/json?placeid=${placeId}&fields=reviews&key=${apiKey}&maxwidth=${pageSize}${
